...
%in noise, assuming unbiased estimator - See "Modern Spectral Estimation" 
%by Stephen Kay, Section 13.4
...
for i = 1:3:3*p
    ...
    %amplitude bound
    bounds(i+1) = (sigma_z * M_inv(i+1,i+1))/2;
    %phase bound
    bounds(i+2) = (sigma_z* M_inv(i+2,i+2))/(2*(theta(i+1)^2));
end
