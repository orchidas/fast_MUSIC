...
for k = 1:length(snr)
    ...
    for m = 1:nsig
        ...
        mse_music(m,k) = norm(err_music(:,m));
        mse_fmusic(m,k) = norm(err_fmusic(:,m));
...
for m = 1:nsig
    ...
    plot(10*log10(snr_s(m,:)), log10(mse_fmusic(m,:)+eps));grid on;hold off;
    xlabel('Spectrum SNR in dB');ylabel('Mean squared error (bins) in dB');
    title(strcat('Frequency of sinusoid = ',num2str(sig_freqs(nsig+m)/(2*pi)),'Hz'));
    %axis([-20,80,-100,40]);
